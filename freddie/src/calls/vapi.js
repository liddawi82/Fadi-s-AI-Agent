...
function buildAssistant({ to, goal, language, calleeName, constraints }) {
  ...
  return {
    ...
    voice: {
      ...
      // Omitted unless VAPI_VOICE_SPEED is set, so the payload is byte-for-byte
      // unchanged by default. Range and clamping live in config.js.
      ...(config.vapi.voiceSpeed !== null ? { speed: config.vapi.voiceSpeed } : {}),
    },
